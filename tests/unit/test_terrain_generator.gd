...
func test_TG01_takao_zoom15() -> void:
	...
	## 期待値は tools/fetch_elevation.py の lat_lon_to_tile と同一式で算出
	var tile: Vector2i = gen.latlon_to_tile(35.6252, 139.2437, 15)
	assert_eq(tile.x, 29058, "高尾山x座標")
	assert_eq(tile.y, 12909, "高尾山y座標")


func test_TG02_fuji_zoom14() -> void:
	## 富士山頂 (35.3606, 138.7274) zoom=14
	## 期待値は tools/fetch_elevation.py の lat_lon_to_tile と同一式で算出
	var tile: Vector2i = gen.latlon_to_tile(35.3606, 138.7274, 14)
	assert_eq(tile.x, 14505, "富士山x座標")
	assert_eq(tile.y, 6469,  "富士山y座標")


func test_TG03_west_edge_lon_minus180() -> void:
	## 経度 -180 はタイル x=0
	var tile: Vector2i = gen.latlon_to_tile(0.0, -180.0, 5)
	assert_eq(tile.x, 0, "経度-180のとき x=0")


func test_TG04_tile_x_within_range() -> void:
	## zoom=4 では x の最大は 2^4 - 1 = 15
	var tile: Vector2i = gen.latlon_to_tile(0.0, 179.9, 4)
	assert_lte(tile.x, 15, "タイルxは 2^zoom-1 以下であること")
	assert_gte(tile.x, 0,  "タイルxは0以上であること")
...
func test_TG13_parse_empty_string() -> void:
	## 空文字列は空配列を返す
	var result: Array = gen._parse_csv("")
